...
class SquatKangarooParams(ParamsBase):
    """
    ...
    href = 0.6  # v0 * DT * TFlyUp / 2  # Height to reach at Tmid

    comRefTrajWeight = 1e5
    href_relative = 0.5
    href_relative_mean = (1 + 0.5) / 2
    href_relative_amplitude = (1 - 0.5) / 2
    ...
    refStateWeight = 1  # /
    refTorqueWeight = 1e-5  # /
    stateTerminalWeight = 1000
    refForceWeight = 0  # 1e5  # /
    copWeight = 0
    ...
    def __init__(self, rmodel):
        ...
        self.comRefTraj = [
            [
                1,
                1,
                self.href_relative_mean
                + self.href_relative_amplitude * np.cos(2 * np.pi * t / self.Ttotal),
            ]
