...
def jump_cleobot_closed(jump_duration, guessFile=None, saveFile=None, benchmark=False):
    # #####################################################################################
    # ### LOAD ROBOT ######################################################################
    # #####################################################################################
    robot = loaders.cleobot_closed()
    params = motionsParams.JumpCleobotParams(robot.model)
...
if __name__ == "__main__":
    ...
    benchmark = False
    if benchmark:
        robot, ddp, sol, params, report = jump_cleobot_closed(0.4, benchmark=True)
        print_bench(report)
        plot_bench(report)
    else:
        robot, ddp, sol, params = jump_cleobot_closed(0.4)
