...
def visualizeConstraints(viz, model, data, constraint_models, q=None):
    ...
    for i, c in enumerate(constraint_models):
        ...
        box = addViewerBox(
            viz, "Constraints/" + name + "_1", 0.03, 0.02, 0.01, [1, 0, 0, 0.5]
        )
        ...
        box = addViewerBox(
            viz, "Constraints/" + name + "_2", 0.03, 0.02, 0.01, [0, 1, 0, 0.5]
        )
        applyViewerConfiguration(
            viz,
            "Constraints/" + name + "_2",
...
def visualizeInertias(viz, model, q, alpha=0.1):
    ...
    for idJoint in range(model.njoints):
        ...
        h = np.sqrt(6 * (I_xx_p + I_yy_p - I_zz_p) / (I.mass)) * alpha
        d = np.sqrt(6 * (I_xx_p - I_yy_p + I_zz_p) / (I.mass)) * alpha
        w = np.sqrt(6 * (-I_xx_p + I_yy_p + I_zz_p) / (I.mass)) * alpha
        ...
        addViewerBox(viz, f"Inertias/I_{model.names[idJoint]}", w, d, h, [0, 1, 0, 1.0])
        applyViewerConfiguration(
            viz, f"Inertias/I_{model.names[idJoint]}", pin.SE3ToXYZQUAT(oMf)
        )
...
def record_trajectory(
    viz, start_pos, qs, path="/tmp/video.mp4", DT=0.015, fixed_cam=False, end_pos=None
):
    import imageio
