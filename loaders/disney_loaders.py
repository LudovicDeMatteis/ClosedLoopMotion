...
def disney_closed(base_height=0.400):
    try:
    ...
    (
        ...
        visual_model,
        collision_model,
    ) = load("disney_2legs", free_flyer=True)
    data = model.createData()

    contactIds = [model.getFrameId("left_foot_frame"), model.getFrameId("right_foot_frame")]
    ...
    MBasePlacement.translation = np.array([0, 0, 0.750])
